@section('css')
    <style>
      ...
      /*   #completed {
        ...
        .radio-tile-group .input-container .icon-completed svg {
            fill: green;
        }
        ...
        .radio-tile-group .input-container .radio-tile-label-completed {
            text-align: center;
            font-size: 0.75rem;
            font-weight: 600;
            text-transform: uppercase;
        ...
        .radio-tile-group .input-container .radio-button:checked + .radio-tile .radio-tile-label-completed {
            color: white;
            background-color: #079ad9;
        }
  ...
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="{{ route('therapist.update.booking.status') }}" method="POST" data-rejection-reason="">
            @csrf
            <div class="modal-body text-center">
                ...
                <div class="radio-tile-group">
                    <div class="input-container">
                        <input id="pending" class="radio-button" type="radio" name="booking_status" value="Pending" />
                    ...
                    <div class="input-container">
                        <input id="approved" class="radio-button" type="radio" name="booking_status" value="Approved" />
                        <div class="radio-tile">
                ...
                <div class="radio-tile-group">
                    ...
                    <div class="input-container completed-container">
                        ...
                        <div class="radio-tile ">
                            ...
                            <label for="drive" class="radio-tile-label-completed">Completed</label>
                        </div>
                    </div>
                </div>
...
<div class="modal fade" data-bs-backdrop='static' id="rejection_reason_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Rejection Reason</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true" onclick="closeModalReason()">×</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="rejectionForm">
                    <input type="hidden" id="booking_id_rejection" name="booking_id">
...
<script>
   ...
   $(document).ready(function() {
    ...
    function openRejectionReasonModal(bookingId, status) {
    $('#booking_id_rejection').val(bookingId);
    $('#booking_status_modal').attr('data-rejection-reason', ''); // Clear rejection reason when opening the modal
    $('#rejection_reason_modal').modal('show');
}

    $('input[name="booking_status"]').click(function() {
        if ($(this).val() === 'Rejected') {
            var bookingId = $('#booking_id').val();
            openRejectionReasonModal(bookingId);
            $(this).prop('checked', false);
        }
    });

    $('#rejectionForm button').click(function() {
        var rejectionReason = $('#rejection_reason').val();
        var bookingId = $('#booking_id_rejection').val();
        ...
        // Update the rejection reason in the booking_status_modal data attribute
        $('#booking_status_modal').attr('data-rejection-reason', rejectionReason);
        $('#' + 'rejected').prop('checked', true);
        // Add your logic to update the rejection reason on the server
    });

    $('#booking_status_modal form').submit(function(e) {
        // Prevent the form from submitting
        e.preventDefault();

        // Get the rejection reason from the data attribute
        var rejectionReason = $('#booking_status_modal').attr('data-rejection-reason');

        // Include the rejection reason in the form data
        $(this).append('<input type="hidden" name="message" value="' + rejectionReason + '">');

        // Now, you can submit the form with the rejection reason included
        this.submit();
    });
    ...
    function closeModalReason() {
        $('#rejection_reason_modal').modal('toggle');
